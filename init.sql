...
(
    ...
    `commentTime`  timestamp(0) NOT NULL COMMENT '评论时间',
...
(
    ...
    `appearTime`   timestamp(0) NOT NULL COMMENT '发生异常的时间',
    PRIMARY KEY (`id`) USING BTREE,
...
(
    ...
    `createTime`     timestamp(0) NOT NULL COMMENT '创建日期',
...
(
    ...
    `firstVisitTime` timestamp(0) NOT NULL COMMENT '第一次访问时间',
...
(
    ...
    `permission`   varchar(511) NOT NULL COMMENT '权限',
    `insertUserId` int(11)      NOT NULL COMMENT '新增用户id',
    `insertTime`   timestamp(0) NOT NULL COMMENT '新增时间',
    `updateUserId` int(11)      NOT NULL COMMENT '修改用户id',
    `updateTime`   timestamp(0) NOT NULL COMMENT '修改时间',
    PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB
  ROW_FORMAT = Compact;
